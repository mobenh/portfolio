      ...
      {nodes.map((node, index, nodesArray) => (
        <React.Fragment key={`line-${node.id}`}>
          ...
          {generateLeafNodes(node, index, nodesArray.length).map((leaf) => {
            ...
            const pathLength = Math.sqrt(Math.pow(leaf.x - lineStartX, 2) + Math.pow(leaf.y - lineStartY, 2));
            const dashArray = pathLength / 10; // Adjust this value to change dash size
            ...
            return (
              <motion.path
                ...
                initial={{ pathLength: 0, strokeDashoffset: pathLength }}
